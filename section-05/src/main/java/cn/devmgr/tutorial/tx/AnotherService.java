...
public class AnotherService {
	...
	public void insert2trans(PersonDto p1, PersonDto p2, boolean throwEception) {
		...
		try {
			...
			// requireNew create a new transaction and hang the former one untill new
			// transaction finished
			// since it is a new transaction, it is result does not influence former
			// transaction for example a exception caused roll back will not influence
			// transaction's update/insert to database
	...
	@Transactional(propagation = Propagation.REQUIRED)
	public void insertNested2(PersonDto p1, PersonDto p2, boolean throwEception) {
		personDao.insert(p1);
		try {
			personService.insertNested(p2, false);
		} catch (Exception e) {
